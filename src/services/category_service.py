...
def add_category_servcie(category: CategorySchema) -> None:
    category_operations.add_category(category)


def get_category_service(category_id: UUID) -> CategorySchema:
    response = category_operations.get_category(category_id)
    return response
